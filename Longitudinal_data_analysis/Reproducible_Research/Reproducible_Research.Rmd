...
**Exercise:** Using any of the datasets available at the `R` package called `datasets` (a complete description of all available examples can be found here [datasets](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/00Index.html), create a
reproducible report that documents the data (dimensions, summary statistics, ...)
and provides a set of visualisations (a PCA plot, `pairs`, ...). To
conclude your report, add a *Session information* section with the
output of `sessionInfo()`. Upload the final pdf file into the Campus Virtual (Entrega de Trabajos - 
Tarea 1 - Reproducible Research)
